...
int PDC_resize_screen(int nlines, int ncols)
{
	...
	pdc_win_nlines = size.Y;
	pdc_win_ncols = size.X;

	extern int pdc_win_ncols;

    return OK;
}

int PDC_get_screen_size(int *nlines, int *ncols)
{
	CONSOLE_SCREEN_BUFFER_INFO csbi;
	if (GetConsoleScreenBufferInfo(GetStdHandle(STD_OUTPUT_HANDLE), &csbi) == 0) return ERR;
