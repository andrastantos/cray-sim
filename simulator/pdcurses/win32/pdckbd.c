...
static HWINEVENTHOOK PDC_hook = 0;

static int PDC_resize_event = 0;

VOID CALLBACK PDC_win_event_proc(
	...
	int ncols, nlines;
	if (hWinEventHook == PDC_hook) {
		switch (event) {
		case EVENT_CONSOLE_LAYOUT:
			if (PDC_get_screen_size(&nlines, &ncols) == OK) {
				if (nlines != pdc_win_nlines || ncols != pdc_win_ncols) {
					pdc_win_nlines = nlines;
					pdc_win_ncols = ncols;
					PDC_resize_event = 1;
				}
			}
...
void PDC_hook_setup(void)
{
	PDC_hook = SetWinEventHook(
		EVENT_MIN,
		EVENT_MAX,
		NULL,
		PDC_win_event_proc,
	...
	if (PDC_hook == 0) {
		// What to do in case of an error? Probably nothing, simply won't handle events
	}
}

void PDC_hook_shutdown(void)
{
	if (PDC_hook != 0) UnhookWinEvent(PDC_hook);
	PDC_hook = 0;
}
...
{
	// Run a message pump so we get the resize events in PDC_win_event_proc
	MSG Msg;
	BOOL RetVal = PeekMessageA(&Msg, NULL, 0, 0, PM_REMOVE);
	if (RetVal) {
    ...
    return (event_count != 0 || PDC_resize_event == 1);
}
...
static int _process_key_event(void)
{
	...
	/* Test to make sure we're in bounds of the key table */
	/* This path interestingly gets triggered (using vk code of 255) only under PGO builds, and after OS returns from the lock screen. */
	/* The actual VK code is undocumented, but the 'internets' thinks it's the code for the 'FN' key on certain laptops. */
	if ((state & ENHANCED_KEY)) {
		if (vk >= (sizeof(kptab) / sizeof(kptab[0]))) {
			return -1;
		}
	}
	...
	/* Test to make sure we're in bounds of the key table */
	/* While I have not been able to hit these cases, better ignoring the key-press than crashing... */
	if (enhanced) {
		if (idx >= (sizeof(ext_kptab) / sizeof(ext_kptab[0]))) {
			return -1;
		}
	}
	else {
		if (idx >= (sizeof(kptab) / sizeof(kptab[0]))) {
			return -1;
		}
	}
